// Iterative Queue based C program to find the height of the given BT
#include <stdio.h>
#include <stdlib.h>
...
/* Given a binary tree, find out the height of the Binary Tree */
int heightOfBinaryTree (struct node *root)
{
  if (!root)
    return 0;
  int rear = -1, front = -1, level = 0;
...
void
enQueue (struct node **queue, int *rear, struct node *new_node)
{
  queue[*rear] = new_node;
  (*rear)++;
}
